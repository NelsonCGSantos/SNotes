...
exports.createNote = async (req, res) => {
  try {
    if (!req.user?.id) return res.status(401).json({ error: "Unauthorized. Please log in." });
...
exports.getNotes = async (req, res) => {
  try {
    ...
    res.json([...userNotes, ...sharedNotesList]);
  } catch (error) {
...
exports.updateNote = async (req, res) => {
  try {
    ...
    const { title, content } = req.body;
    if (!title && !content) return res.status(400).json({ error: "Provide at least one field to update." });

    const updated = await Note.update(req.body, { where: { id: req.params.id, userId: req.user.id } });
    if (!updated[0]) return res.status(404).json({ error: "Note not found or no changes made" });
...
exports.shareNote = async (req, res) => {
  try {
    ...
    if (!userId) return res.status(400).json({ error: "Shared user ID is required" });
...
exports.searchNotes = async (req, res) => {
  try {
    ...
    const sharedNotes = await SharedNote.findAll({
      where: { sharedWithUserId: req.user.id },
      include: [{ model: Note, as: "sharedNote", where: { [Op.or]: [{ title: { [Op.iLike]: `%${query}%` } }, { content: { [Op.iLike]: `%${query}%` } }] } }],
    });
